/* eslint-disable import/no-unresolved,no-restricted-globals,import/no-extraneous-dependencies */
...
$('#banner-preview-player').click(bannerPreviewHandler(
	'player',
	() => `/api/player/${$('#banner-name').val()}`, (res) => {
	...
	}, () => `player?name=${encodeURIComponent($('#banner-name').val())}&theme=${$('#banner-theme-player').val()}`,
));

$('#banner-preview-server').click(bannerPreviewHandler(
	'server',
	() => `/api/server/${$('#banner-host').val()}/${$('#banner-port').val()}`, res => Handlebars.compile(codeMirrorServer.getValue())({ server: res }), () => `server?host=${$('#banner-host').val()}&port=${$('#banner-port').val()}&theme=${$('#banner-theme-server').val()}`,
));

$('#banner-preview-clan').click(bannerPreviewHandler(
	'clan',
	() => `/api/clan/${encodeURIComponent($('#banner-clantag').val())}`, (res) => {
	...
	}, () => `clan?clantag=${encodeURIComponent($('#banner-clantag').val())}&theme=${$('#banner-theme-clan').val()}`,
));
...
function bannerGenerateHandler(type, getTemplate, getIMGURL) {
	return function () {
		...
		if (theme === 'custom') {
			$.post(`${bannerURL}register`, { type, template: getTemplate() }).then((res) => {
...
$('#banner-generate-player').click(bannerGenerateHandler(
	'player',
	() => codeMirrorPlayer.getValue(), theme => `player?name=${encodeURIComponent($('#banner-name').val())}&theme=${theme}`,
));

$('#banner-generate-server').click(bannerGenerateHandler(
	'server',
	() => codeMirrorServer.getValue(), theme => `server?host=${$('#banner-host').val()}&port=${$('#banner-port').val()}&theme=${theme}`,
));

$('#banner-generate-clan').click(bannerGenerateHandler(
	'clan',
	() => codeMirrorClan.getValue(), theme => `clan?clantag=${encodeURIComponent($('#banner-clantag').val())}&theme=${theme}`,
));
