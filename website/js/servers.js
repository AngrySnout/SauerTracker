...
function renderServers() {
  ...
  _.each(servers, server => {
    if (
      lookFor &&
      _.find(server.players, pl => pl.toLowerCase().indexOf(lookFor) >= 0)
    ) {
  ...
  if (lookingForServer) {
    ...
    servs = _.union(
      filter(servers, lookingForServer, { key: 'description' }),
      _.filter(servers, serv => serv.fullText.indexOf(lookingForLower) >= 0)
    );
  }
...
function renderGame() {
  ...
  _.each(openGame.players, player => {
    if (lookFor && player.name.toLowerCase().indexOf(lookFor) >= 0)
...
function loadServer(host, port) {
  $.get(`/api/server/${host}/${port}`, result => {
    if (!openServerHost || openServerHost !== host || openServerPort !== port)
      return;
    openGame = result;
    renderGame();
  });
...
window.showServer = function(host, port) {
  $('#server-info-content').html(
    '<div style="text-align: center; margin-top: 4em;"><i class="fa fa-spinner fa-pulse fa-4x"></i></div>'
  );
