...
window.dateReflow = function() {
  ...
  $('.date').each(function() {
    ...
    else if (cd.diff(now, 'days') >= -7)
      $(this).text(cd.fromNow() + cd.format(', HH:mm'));
    else if (!cd.isSame(now, 'year'))
      $(this).text(cd.format('MMMM Do YYYY, HH:mm'));
    else if (cd.diff(now, 'days') < -7)
      $(this).text(cd.format('MMMM Do, HH:mm'));
...
window.tryLoadBackground = function(name) {
  ...
  bg.onload = function() {
    $('body').css(
      'background',
      `url(/images/mapshots/${name}.jpg) no-repeat center center fixed`
    );
