...
function populateRanks() {
  database
    .raw(
      'BEGIN; CREATE TABLE playerranks2 AS SELECT ranked.*, rank() OVER (ORDER BY frags DESC) AS rank FROM (SELECT name, frags FROM players) AS ranked ORDER BY rank ASC; CREATE INDEX ON playerranks2 (name); DROP TABLE IF EXISTS playerranks; ALTER TABLE playerranks2 RENAME TO playerranks; COMMIT;'
    )
    .catch(err => {
      logError(err);
    });
...
export function getDuels(name) {
  return database
    ...
    .from('games')
    .where('gametype', 'duel')
    .where('meta', 'LIKE', `%"${escapePostgresLike(name)}"%`)
    .then(rows => {
      const res = {
        total: rows.length,
        wins: 0,
...
export function getPlayer(name) {
  if (playerManager.banNames[name])
    return Promise.reject(new ObjectBannedError());
  ...
  return database('players')
    ...
    .then(rows => {
      if (!rows.length) {
        ...
        const row = rows[0];
        try {
          row.efficstats = row.efficstats
            ? JSON.parse(row.efficstats)
            : [0, 0, 0, 0, 0];
          row.instastats = row.instastats
            ? JSON.parse(row.instastats)
            : [0, 0, 0, 0, 0];
        } catch (e) {
          logWarn(e);
        ...
        if (row.efficstats[0] > 0)
          row.efficstats.push(row.efficstats[4] / row.efficstats[0]);
        if (row.instastats[0] > 0)
          row.instastats.push(row.instastats[4] / row.instastats[0]);
        row.efficstats[4] = round2(
          row.efficstats[0] / (row.efficstats[2] || 1)
        );
        row.instastats[4] = round2(
          row.instastats[0] / (row.instastats[2] || 1)
        );
        ...
        return Promise.all([
          getTotalGames(name),
          getLastGames(name),
          database('playerranks').where({ name }),
          getDuels(name),
        ]).then(results => ({
          player: row,
...
app.get('/api/player/:name', (req, res) => {
  getPlayer(req.params.name)
    ...
    .catch(ObjectNotFoundError, () => {
      res.status(404).send({ error: 'No player found with this name.' });
    })
    .catch(ObjectBannedError, () => {
      res.status(400).send({ error: 'This player name is banned.' });
    })
    .catch(err => {
...
app.get('/api/player/activity/:name', (req, res) => {
  const thisMonth = moment()
    ...
    .format('YYYY-MM-DD');
  database
    .select(database.raw("date_trunc('day', timestamp) as date"))
    .count('* as count')
    .from('games')
