...
export function findPlayers(name, country) {
  ...
  const query = database('players').where(
    'name',
    'ilike',
    `%${escapePostgresLike(name)}%`
  );
  if (country) {
...
export function makeTeams(names, mode, map) {
  ...
  query1 = query1
    .whereRaw("games.timestamp > CURRENT_DATE - INTERVAL '3 months'")
    .whereIn('stats.name', names)
  ...
  return Promise.join(query1, query2, (players, avgStats) => {
    ...
    function compareTeamPairs(a, b) {
      ...
      return (
        aDeltaFrag + aDeltaFlag + Math.abs(aDeltaFrag - aDeltaFlag) <
    ...
    return {
      good: _.map(teams[0].players, 'name'),
      evil: _.map(teams[1].players, 'name'),
    };
  });
}
