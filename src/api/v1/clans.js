...
cache.set('clans', 60 * 60 * 1000, () => database('games').where({ gametype: 'clanwar' }).select('meta').then((rows) => {
  const clans = {};
  _.each(rows, (row) => {
    try {
  ...
  const clns = _.orderBy(_.map(vars.clans, (clan) => {
    ...
    const draws = ((clans[clan.tag] || 0) - ((wins[clan.tag] || 0) + (losses[clan.tag] || 0)));
    const rate = (clans[clan.tag] ? (wins[clan.tag] + draws / 2) / clans[clan.tag] : 0);
    return {
      name: clan.tag, wins: wins[clan.tag], losses: losses[clan.tag], ties: draws, rate: round2(rate), points: round2((wins[clan.tag] + draws / 2) * rate),
    };
  }), 'points', 'desc');
  let rank = 1;
  _.each(clns, (clan) => {
