...
function getLatestGames(clan) {
	return database('games').where('meta', 'like', `%${escapePostgresLike(clan.replace(/"/g, '\\"'))}%`).where({ gametype: 'clanwar' }).orderBy('id', 'desc')
		.limit(10)
		.then((rows) => {
			_.each(rows, (game) => {
				if (game.meta) {
...
function getLatestMembers(clan) {
	return database('spy').where('name', 'ilike', `%${escapePostgresLike(clan)}%`).max('lastseen as lastseen').select('name')
		.groupBy('name')
...
export function getClan(name) {
	return Promise.all([cache.get('clans'), getLatestGames(name), getLatestMembers(name)])
		.spread((clans, latestGames, latestMembers) => {
			const clan = _.find(clans, { name });
			if (!clan) throw new ObjectNotFoundError();

			const info = _.find(vars.clans, { tag: name });
			return {
				clan, info, games: latestGames, members: latestMembers,
			};
		});
}

app.get('/api/clan/:name', (req, res) => {
