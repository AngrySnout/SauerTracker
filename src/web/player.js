...
import playerTemplate from '../../website/views/player.pug';

app.get('/player/:name', (req, res) => {
  getPlayer(req.params.name)
    .then(result => {
      res.send(playerTemplate(_.assign(result, { _, vars })));
    })
    ...
    .catch(ObjectBannedError, () => {
      res
        .status(400)
        .render('error', { status: 400, error: 'This player name is banned.' });
    })
    .catch(err => {
