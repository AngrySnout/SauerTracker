...
app.get('/server/:host/:port', function (req, res) {
	findServer(req.params.host, req.params.port)
		.then(server => { res.render('server', { server: server, vars: vars, _: _, bannerURL: vars.bannerURL }); })
		.catch(ObjectNotFoundError, () => { res.status(404).render('error', { status: 404, error: 'Server not found.' }); })
