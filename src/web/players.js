...
cache.set('top-fraggers', 60 * 60 * 1000, () =>
  database
    .select('stats.name as name')
...
cache.set('top-runners', 60 * 60 * 1000, () =>
  database
    .select('stats.name as name')
    ...
    _.each(rows, row => {
      if (row.meta) {
        try {
          ...
          duelists[row.meta[0]] =
            (duelists[row.meta[0]] && duelists[row.meta[0]] + 1) || 1;
          duelists[row.meta[2]] =
            (duelists[row.meta[2]] && duelists[row.meta[2]] + 1) || 1;
        } catch (e) {
          row.meta = null;
        }
      }
...
cache.set('top-duelists-daily', 2 * 60 * 60 * 1000, () =>
  duelsSince(
    moment()
      .startOf('day')
      .format('YYYY-MM-DD')
...
cache.set('top-duelists-weekly', 2 * 60 * 60 * 1000, () =>
  duelsSince(
    moment()
      .subtract(7, 'days')
      .startOf('day')
...
cache.set('top-duelists-monthly', 2 * 60 * 60 * 1000, () =>
  duelsSince(
    moment()
      .subtract(30, 'days')
      .startOf('day')
...
cache.set('player-countries', 2 * 60 * 60 * 1000, () =>
  database('players')
    .select('country')
    .count('* as count')
    .groupBy('country')
