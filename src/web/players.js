...
cache.set('top-players', 60 * 60 * 1000, () => database.select('stats.name as name').count('*').from('stats').join('games', 'games.id', 'stats.game')
...
cache.set('top-players-daily', 60 * 60 * 1000, () => {
  ...
  return database.select('stats.name as name').count('*').from('stats').join('games', 'games.id', 'stats.game')
...
cache.set('top-fraggers', 60 * 60 * 1000, () => database.select('stats.name as name').sum('stats.frags as frags').from('stats').join('games', 'games.id', 'stats.game')
  .whereNotIn('name', _.keys(playerManager.banNames))
  .groupBy('name')
  .orderBy('frags', 'desc')
  .limit(15));
...
cache.set('top-fraggers-daily', 60 * 60 * 1000, () => {
  const start = moment().startOf('day').format('YYYY-MM-DD HH:mm:ss');
  return database.select('stats.name as name').sum('stats.frags as frags').from('stats').join('games', 'games.id', 'stats.game')
    .where('games.timestamp', '>', start)
    .whereNotIn('name', _.keys(playerManager.banNames))
    .groupBy('name')
    .orderBy('frags', 'desc')
...
cache.set('top-runners', 60 * 60 * 1000, () => database.select('stats.name as name').sum('stats.flags as flags').from('stats').join('games', 'games.id', 'stats.game')
  .whereNotIn('name', _.keys(playerManager.banNames))
  .groupBy('name')
  .orderBy('flags', 'desc')
  .limit(15));
...
cache.set('top-runners-daily', 60 * 60 * 1000, () => {
  const start = moment().startOf('day').format('YYYY-MM-DD HH:mm:ss');
  return database.select('stats.name as name').sum('stats.flags as flags').from('stats').join('games', 'games.id', 'stats.game')
    .where('games.timestamp', '>', start)
    .whereNotIn('name', _.keys(playerManager.banNames))
    .groupBy('name')
    .orderBy('flags', 'desc')
...
function duelsSince(date) {
  ...
  query = query.select('meta').then((rows) => {
    ...
    return _.take(_.orderBy(_.map(duelists, (games, player) => ({ name: player, wins: wins[player] || 0, games })), ['wins', 'games'], ['desc', 'asc']), 15);
  });
  return query;
}
...
cache.set('top-duelists-daily', 2 * 60 * 60 * 1000, () => duelsSince(moment().startOf('day').format('YYYY-MM-DD')));
...
cache.set('player-countries', 2 * 60 * 60 * 1000, () => database('players').select('country').count('* as count').groupBy('country')
  .then((rows) => {
    let sumUnknown = 0;
    const newList = [];
  ...
  findPlayers(req.query.name, req.query.country)
    .then((results) => {
      cache.get('player-countries').then((countries) => {
		        res.render('players', {
          results, name: req.query.name, _, countries,
        });
      });
    })
