...
cache.set('latest-duels', 10 * 60 * 1000, () => database('games').where({ gametype: 'duel' }).orderBy('timestamp', 'desc').limit(10)
	.then((rows) => {
		_.each(rows, (row) => {
			try {
...
cache.set('latest-clanwars', 10 * 60 * 1000, () => database('games').where({ gametype: 'clanwar' }).orderBy('timestamp', 'desc').limit(10)
	.then((rows) => {
		_.each(rows, (row) => {
			try {
				row.meta = JSON.parse(row.meta);
...
app.get('/games', (req, res) => {
	Promise.all([cache.get('latest-duels'), cache.get('latest-clanwars')])
		.then((results) => {
			res.render('games', {
				vars, _, latestDuels: results[0], latestClanwars: results[1],
			});
		})
		.catch((err) => {
