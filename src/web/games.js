...
import gamesTemplate from '../../website/views/games.pug';

cache.set('latest-duels', 10 * 60 * 1000, () =>
  database('games')
    .where({ gametype: 'duel' })
...
cache.set('latest-clanwars', 10 * 60 * 1000, () =>
  database('games')
...
app.get('/games/find', (req, res) => {
  findGames(req.query).then(result => {
    ...
    if (result.results.length) {
      ...
      nextPage =
        result.results[result.results.length - 1].id > result.stats.min;
    }
    res.send(
      gamesTemplate(
        _.assign(
          {
            ...
            prevPageURL: prevPage
              ? prevPageURL(req, result.results[0].id)
              : null,
            nextPageURL: nextPage
              ? nextPageURL(req, result.results[result.results.length - 1].id)
              : null,
          },
          result
        )
