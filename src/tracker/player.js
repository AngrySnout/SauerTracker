...
const emptyStats = { frags: 0, flags: 0, deaths: 0, tks: 0, accFrags: 0 };

export function totalStats(modeStats) {
	let res = Object.assign({}, emptyStats);
	_.each(_.keys(modeStats), function (key) {
...
export function sumModeStats(newModeStats, oldModeStatsString) {
	...
	let oldModeStatsArray = JSON.parse(oldModeStatsString);
	let newModeStatsArray = [newModeStats.frags, newModeStats.flags, newModeStats.deaths, newModeStats.tks, newModeStats.accFrags];
...
export function updateRow(name, row, modeStats, country) {
	let newStats = totalStats(modeStats);
	
	// TODO update database schema (remove acc, kpd, and countryName fields, remove 5th element from *stats and multiply last one by frags)
	let stats = {
		name: row.name||name,
		frags: row.frags||0,
		flags: row.flags||0,
	...
	if (modeStats.insta) stats.instastats = sumModeStats(modeStats.insta, stats.instastats);
	if (modeStats.effic) stats.efficstats = sumModeStats(modeStats.effic, stats.efficstats);
...
export default class Player {
	...
	updateState(gameMode, newState, oldState) {
		...
		if (!this.modeStats[modetype]) this.modeStats[modetype] = Object.assign({}, emptyStats);
		
		let newfrags = Math.max(newState.frags-oldState.frags, 0);
		this.modeStats[modetype].frags += newfrags;
		this.modeStats[modetype].flags += Math.max(newState.flags-oldState.flags, 0);
		this.modeStats[modetype].deaths += Math.max(newState.deaths-oldState.deaths, 0);
		this.modeStats[modetype].tks += Math.max(newState.tks-oldState.tks, 0);
		this.modeStats[modetype].accFrags += newState.acc * newfrags;
	}

	saveStats(row, trx) {
		let stats = updateRow(this.name, row||{}, this.modeStats, this.country);
