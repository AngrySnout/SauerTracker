...
export default class Player {
	...
	updateState(server, newState, oldState, curTime) {
		...
		if (newState.frags > 0) {
			...
			this.modes[modetype].acc = ((this.modes[modetype].acc * this.modes[modetype].frags)
					+ (newState.acc * newfrags)) / Math.max(this.modes[modetype].frags + newfrags, 1);
	...
	saveSpy() {
		const self = this;
		return _.map(this.ips, (info, ip) => database('spy').where({ name: self.name, ip }).then((rows) => {
			if (!rows.length) {
				return database('spy').insert({
					name: self.name,
					ip: self.ip,
					country: self.country,
					lastseen: info.lastSeen,
					lshost: info.onServer.host,
					lsport: info.onServer.port,
				}).then();
			}
			return database('spy').where({ name: self.name, ip }).update({
				country: self.country,
				lastseen: info.lastSeen,
				lshost: info.onServer.host,
				lsport: info.onServer.port,
	...
	sumStats() {
		...
		_.each(_.keys(this.modes), (key) => {
			if (res.frags + self.modes[key].frags !== 0) {
				res.acc = ((res.acc * res.frags)
						+ (self.modes[key].acc * self.modes[key].frags)) / (res.frags + self.modes[key].frags);
			}
	...
	getModeStats(mode) {
		if (!this.modes[mode]) return undefined;
		return [this.modes[mode].frags,
			this.modes[mode].flags,
			this.modes[mode].deaths,
			this.modes[mode].tks,
			round2(this.modes[mode].frags / Math.max(this.modes[mode].deaths, 1)),
			this.modes[mode].acc];
	}
	...
	saveStats(row, trx) {
		...
		if (newStats.frags + stats.frags === 0) stats.acc = 0;
			...
			stats.acc = round2(((newStats.frags * newStats.acc) + (stats.frags * stats.acc)) /
				(newStats.frags + stats.frags));
		}
		...
		function calcModeStats(self, mode) {
			const newModeStats = self.getModeStats(mode);
			const modeStats = JSON.parse(mode === 'insta' ? stats.instastats : stats.efficstats);
			if (modeStats[0] + newModeStats[0] === 0) modeStats[5] = 0;
			else {
				modeStats[5] = round2((modeStats[0] * modeStats[5] + newModeStats[0] * newModeStats[5]) /
					(modeStats[0] + newModeStats[0]));
			}
		...
		return database('players').insert(stats).transacting(trx).then();
	}
}
