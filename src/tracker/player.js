    ...
    return _.map(this.ips, (info, ip) => database('spy').where({ name: self.name, ip }).then((rows) => {
      if (!rows.length) {
        return database('spy').insert({
          name: self.name, ip: self.ip, country: self.country, lastseen: info.lastSeen, lshost: info.onServer.host, lsport: info.onServer.port,
        }).then();
      }
      return database('spy').where({ name: self.name, ip }).update({
        country: self.country, lastseen: info.lastSeen, lshost: info.onServer.host, lsport: info.onServer.port,
      }).then();
    }));
  }
